...
- **Correspondance des catégories à l'import (tableur)** : si le fichier a une colonne « Catégorie » (et/ou « Sous-catégorie »), correspondance exacte (insensible casse/accents/espaces) → sinon Gemini sur le **seul nom de catégorie** (`GeminiCat.matchCategories`, cache `invest_cat_map_cache` consultable/éditable dans les paramètres) → sinon fallback sur le libellé. Origine indiquée dans l'aperçu par pastille (🟢 fichier · 🟡 approchée · 🔵 IA · ⚪ à catégoriser).
