const Charts = {
  ...
  // Dégradé vertical propre et sobre PAR BARRE (vif en haut → ~40% plus sombre en bas) + fin
  // highlight blanc sur le bord supérieur. Peint directement en canvas plutôt que via un
  // `backgroundColor` scriptable (_vGrad) : celui-ci est relatif à TOUTE la zone du graphique,
  // donc une petite barre assise en bas du graphique n'affiche presque que la couleur sombre —
  // ici chaque barre montre sa propre transition claire→sombre quelle que soit sa hauteur.
  _barGradientPlugin() {
    const RECIPES = {
      'Revenus':  ['#00b37e', '#004d35'],
      'Dépenses': ['#e53e3e', '#5a0f0f'],
    };
    return {
      id: 'barGradient',
      afterDatasetDraw(chart, args) {
        ...
        if (!recipe) return;
        const { ctx } = chart;
        const [top, bottom] = recipe;

        args.meta.data.forEach((el) => {
  ...
  // épaisse + aire dégradée (couleur → transparent) vers zéro.
  fluxBar(labels, revData, depData, soldeData) {
    ...
    this.create('chart-flux-bar', {
      ...
      options: {
        ...
        // Gridlines pointillées très discrètes (propres à ce graphique, _yAxis() reste inchangé
        // pour les autres graphiques qui le partagent).
        scales: {
          y: this._yAxis({ grid: { color: '#ffffff08' }, border: { display: false, dash: [3, 3] } }),
          x: this._xAxis(),
        },
      },
      plugins: [this._barShadowPlugin(), this._barGradientPlugin()],
    });
  },
