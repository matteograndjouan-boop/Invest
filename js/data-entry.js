const DataEntry = {
  ...
  _confirmReassign(event) {
    ...
    // reassignedFrom mémorise le bucket ('expense'/'revenue'), catégorie et sous-catégorie
    // AVANT ce changement (écrasé si déjà présent — un seul niveau d'annulation, le plus
    // récent) + le scope réellement appliqué ('subcat'/'cat') : alimente le badge 🏷️ (colonne
    // catégorie ou sous-catégorie selon la portée) et undoReassign(), y compris pour rebasculer
    // de bucket au besoin. save() (édition manuelle classique) remplace l'objet entier et efface
    // donc ce marqueur automatiquement — une édition déclarée prend le pas sur l'annulation
    // d'une réaffectation en masse.
    if (scope === 'subcat') {
      ...
      // simple mutation en place.
      const newSub = fd.get('subcat_only') || '';
      targets.forEach(({ id, type }) => {
        const rec = findRec(id, type);
        if (!rec) return;
        rec.reassignedFrom = { type, category: rec.category, subcategory: rec.subcategory || '', scope: 'subcat' };
      ...
      targets.forEach(({ id, type: oldBucket }) => {
        ...
        if (!rec) return;
        const reassignedFrom = { type: oldBucket, category: rec.category, subcategory: rec.subcategory || '', scope: 'cat' };
  ...
  // Annule une réaffectation (clic sur le badge 🏷️ posé par _confirmReassign) : restaure bucket,
  // catégorie et sous-catégorie mémorisés dans reassignedFrom et efface le marqueur, donc le
  // badge disparaît — un seul niveau d'annulation (la dernière réaffectation en date). `type` =
  // bucket ACTUEL de la ligne (voir row._type dans render), pas forcément celui d'avant.
  undoReassign(id, type) {
    const list = type === 'expense' ? Storage.getExpenses() : Storage.getRevenues();
    const rec = list.find(r => r.id === id);
    if (!rec || !rec.reassignedFrom) return;
    const prev = rec.reassignedFrom;
    ...
    if (prev.type === type) {
      rec.category = prev.category;
      rec.subcategory = prev.subcategory || '';
      ...
      const restored = { ...rec, id: Utils.generateId(), category: prev.category, subcategory: prev.subcategory || '' };
      delete restored.reassignedFrom;
      if (prev.type === 'revenue') delete restored.effectiveDate;
      const otherList = prev.type === 'expense' ? Storage.getExpenses() : Storage.getRevenues();
      otherList.push(restored);
      Storage[type === 'expense' ? 'saveExpenses' : 'saveRevenues'](list);
      Storage[prev.type === 'expense' ? 'saveExpenses' : 'saveRevenues'](otherList);
    }
    this.render();
    Dashboard.render();
  },
  ...
  render() {
    ...
    tbody.innerHTML = rows.map(row => {
      ...
      // un handler mousedown pour le glisser-sélectionner. Dépenses ET revenus peuvent être
      // réaffectés (openReassign gère les deux buckets) — plus de restriction aux dépenses.
      const reassignScope = this._reassignScope(row);
      const showReassignBadge = !sel && !!reassignScope;
      const catReassignBadge = (showReassignBadge && reassignScope === 'cat')
        ? `<span class="reassign-badge" title="Catégorie réaffectée — était : ${row.reassignedFrom.category}${row.reassignedFrom.subcategory ? ' / ' + row.reassignedFrom.subcategory : ''}. Cliquer pour annuler." onclick="DataEntry.undoReassign('${row.id}','${row._type}');event.stopPropagation()">🏷️</span>`
        : '';
      const subReassignBadge = (showReassignBadge && reassignScope === 'subcat')
        ? `<span class="reassign-badge" title="Sous-catégorie réaffectée — était : ${row.reassignedFrom.subcategory || '—'}. Cliquer pour annuler." onclick="DataEntry.undoReassign('${row.id}','${row._type}');event.stopPropagation()">🏷️</span>`
        : '';
