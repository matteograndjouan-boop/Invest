  ...
  _cashLedger(envOps, cutoff) {
    ...
    sorted.forEach(op => {
      if (this._CASH_IN.includes(op.type)) {
        ...
        // négatif de même montant, donnant une valeur totale à 0 € qui faisait disparaître
        // l'enveloppe des totaux (retour utilisateur : "il ajoute pas la valeur du PEA"). N'affecte
        // aucune enveloppe déjà correctement financée (le plancher ne joue que si cash − coût < 0,
        // donc aucun changement quand un versement suffisant précède l'achat). Contrairement à
