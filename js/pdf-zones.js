...
const PdfZones = {
  ...
  async start(file) {
    try {
      ...
      const zones = tpl ? JSON.parse(JSON.stringify(tpl)) : (this._autoDetect(fracItems1) || this._defaultZones());

      this._state = { file, pdf, zones, sig, recognized: !!tpl, pageNum: 1 };
      Modal.close();
  ...
  async _renderEditor() {
    const { pdf, pageNum, zones, recognized } = this._state;
  ...
  _refreshOverlay() {
    ...
    if (el) el.innerHTML = this._overlayHTML(this._state.zones);
  },
  ...
  _attachDrag() {
    ...
    stage.addEventListener('pointerdown', (e) => {
      ...
      const startZones = JSON.parse(JSON.stringify(this._state.zones));
  ...
  _attachControls() {
    ...
    if (date2Toggle) {
      date2Toggle.checked = z.date2Enabled;
      document.getElementById('pdfz-date2-which').style.display = z.date2Enabled ? '' : 'none';
      date2Toggle.addEventListener('change', e => {
        z.date2Enabled = e.target.checked;
        if (z.date2Enabled && !z.date2) {
    ...
    document.querySelectorAll('input[name="pdfz-which-date"]').forEach(r => {
      r.checked = (r.value === '2') === (z.dateChoice === 'date2');
      r.addEventListener('change', e => { if (e.target.checked) z.dateChoice = e.target.value === '2' ? 'date2' : 'date'; });
    });
    document.querySelectorAll('input[name="pdfz-amt-mode"]').forEach(r => {
      r.checked = r.value === z.amountMode;
      r.addEventListener('change', e => {
        if (!e.target.checked) return;
        z.amountMode = e.target.value;
        if (z.amountMode === 'split' && !z.debit) {
          const base = z.amount || { x0: 0.7, x1: 0.9 };
          const mid  = (base.x0 + base.x1) / 2;
          z.debit  = { x0: base.x0, x1: mid };
          z.credit = { x0: mid,    x1: base.x1 };
        } else if (z.amountMode === 'single' && !z.amount) {
  ...
  async _extract(pdf, zones) {
    ...
    for (const row of allRows) {
      ...
      if (hasDate) {
        ...
        const rawDate = (zones.date2Enabled && zones.dateChoice === 'date2')
  ...
  async confirm() {
    const { pdf, zones, sig } = this._state;
    ...
    `);
    try {
      const { scanned, transactions } = await this._extract(pdf, zones);
      if (scanned) {
        Modal.close();
