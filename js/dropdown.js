...
const Dropdown = {
  ...
  _open(wrap) {
    ...
    // Ouvre du côté (haut/bas) qui a le plus de place, et étire le panneau jusqu'à occuper tout
    // cet espace disponible (max-height dynamique, pas un plafond fixe) — une liste réaliste
    // (jusqu'à ~15-20 catégories) tient alors entièrement à l'écran sans avoir besoin de
    // défiler. overflow-y:auto (CSS) reste un filet de sécurité pour les listes qui dépassent
    // même ce budget-là (mapping de colonnes à l'import bancaire, etc.).
    const MARGIN = 12;
    const spaceBelow = window.innerHeight - r.bottom - MARGIN;
    const spaceAbove = r.top - MARGIN;
    const openUp = spaceBelow < spaceAbove;
    panel.style.maxHeight = Math.max(openUp ? spaceAbove : spaceBelow, 120) + 'px';
  ...
  init() {
    ...
    // Un scroll (page ou conteneur interne, d'où `true` = phase de capture) rend la position déjà
    // calculée du panneau obsolète — le refermer est plus simple/sûr que la recalculer en
    // continu. Exception : le défilement DANS le panneau lui-même (sa propre liste d'options,
    // overflow-y:auto quand elle dépasse la place disponible) ne doit surtout pas le refermer —
    // sinon impossible de faire défiler la liste, elle se ferme dès le 1er pixel de scroll.
    window.addEventListener('scroll', (e) => {
      if (this._openEl && this._openEl.contains(e.target)) return;
      this._closeAll();
    }, true);
