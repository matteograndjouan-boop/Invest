const Flux = {
  ...
  // Catégories regroupées dans "Autres" sur le donut, seulement au-delà de 7 catégories réelles
  // (sinon la 7e catégorie garde son vrai nom — voir _categoryBreakdown).
  _autresCategories() {
    const entries = this._categoryTotals();
    return entries.length > 7 ? entries.slice(6).map(([label]) => label) : [];
  },
  ...
  // Répartition pour le donut : la légende a la place pour 7 lignes. Jusqu'à 7 catégories
  // réelles, toutes s'affichent sous leur vrai nom. Au-delà (8+), "Autres" regroupe la 7e et
  // les suivantes (donc toujours au moins 2 catégories groupées, jamais un simple renommage).
  _categoryBreakdown() {
