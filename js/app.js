    ...
    if (ext === 'json') {
      ...
      reader.onload = (evt) => {
        try {
          ...
          const dateIdx    = idx(['date treso', 'date']);
          const catIdx     = idx(['categorie']);
          const subCatIdx  = idx(['sous-categorie', 'sous categorie', 'sous']);
          const commentIdx = idx(['commentaire']);
          const valeurIdx  = idx(['valeur']);
          const positifIdx = idx(['positif']);

          const CAT_MAP = {
            'epargne': 'Épargne', 'transport': 'Transport', 'sante': 'Santé',
            'loisir': 'Loisirs', 'loisirs': 'Loisirs', 'alimentation': 'Alimentation',
            'abonnements': 'Abonnements', 'divers': 'Autre', 'logement': 'Logement',
            'vetements': 'Vêtements', 'education': 'Éducation', 'restaurants': 'Restaurants',
          };
          ...
          for (let i = 1; i < rows.length; i++) {
            ...
            const subCat  = String(row[subCatIdx]  || '').trim();
            ...
            existing.push({
              ...
              description,
              amount,
              category: CAT_MAP[catNorm] || 'Autre',
