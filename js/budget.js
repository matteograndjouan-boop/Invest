const Budget = {
  ...
  _themeForm() {
    ...
    const catCheckboxes = cats.map(c =>
      `<label class="cat-checkbox-opt"><input type="checkbox" name="categories" value="${c.name}"> ${c.name}</label>`
    ).join('');
    return `
      <form onsubmit="Budget.saveTheme(event)">
        <div class="form-grid">
          ...
          <div class="form-group form-full">
            ...
            <div class="cat-checkbox-grid">${catCheckboxes || '<em style="color:var(--text-muted)">Aucune catégorie définie — créez-en dans l\'onglet Données > Catégories.</em>'}</div>
          </div>
        </div>
