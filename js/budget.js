const Budget = {
  ...
  _renderList() {
    ...
    // KPI overview — même carcasse "carte fumée" que Flux/Comparaisons (.flux-kpi-card/.fk-smoke),
    // en réutilisant directement les 4 teintes sémantiques déjà établies (fk-balance/fk-expense/
    // fk-revenue/fk-savings) plutôt que de dupliquer des recettes de dégradé : Budget total =
    // référence neutre (bleu), Dépensé = une dépense (rouge), Restant = ressource positive (vert)
    // qui bascule en rouge/alerte dès que ça devient un Dépassement, Dépassements = 4e teinte
    // (rose) comme le Taux d'épargne de Flux.
    if (overviewBar) {
      const isOver = totalRemain < 0;
      const remainIcon = isOver
      ...
      overviewBar.innerHTML = `
        <div class="flux-kpi-card fk-balance">
          <div class="fk-smoke"></div>
          <span class="fk-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg></span>
          <div class="kpi-label">Budget total</div>
          <div class="kpi-value">${Utils.formatCurrency(totalPlanned)}</div>
          <div class="kpi-trend">Prévu · ${PeriodFilter.getLabel()}</div>
        </div>
        <div class="flux-kpi-card fk-expense">
          <div class="fk-smoke"></div>
          <span class="fk-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg></span>
          <div class="kpi-label">Dépensé</div>
          <div class="kpi-value">${Utils.formatCurrency(totalSpent)}</div>
          <div class="kpi-trend">${globalPct.toFixed(0)}% du budget</div>
        </div>
        <div class="flux-kpi-card ${isOver ? 'fk-expense' : 'fk-revenue'}">
          <div class="fk-smoke"></div>
          <span class="fk-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${remainIcon}</svg></span>
          <div class="kpi-label">${isOver ? 'Dépassement' : 'Restant'}</div>
          <div class="kpi-value">${Utils.formatCurrency(Math.abs(totalRemain))}</div>
          <div class="kpi-trend">${isOver ? 'Au-dessus du prévu' : 'Encore disponible'}</div>
        </div>
        <div class="flux-kpi-card fk-savings">
          <div class="fk-smoke"></div>
          <span class="fk-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></span>
    ...
    // (contrairement aux cartes juste en dessous, qui gardent leur ordre habituel). TOUTES les
    // catégories ayant une dépense sur la période, pas seulement celles avec un budget déjà fixé
    // (les autres graphiques par catégorie de l'app — Flux, Comparaisons — ne filtrent pas non
    // plus sur "a un budget") : planned=0 pour une catégorie sans thème, _budgetOutlinePlugin
    // (voir charts.js) ignore déjà silencieusement un budget à 0/absent, donc juste une barre
    // pleine sans contour pour ces catégories-là.
    const trackedCatNames = new Set(themes.flatMap(t => t.categories || []));
    const untrackedNames  = [...new Set(expenses.map(e => e.category))].filter(name => !trackedCatNames.has(name));
    const chartSorted = [
      ...themes.map(theme => ({ name: theme.name, spent: this._computeSpent(theme, expenses), planned: this._plannedForPeriod(theme) })),
      ...untrackedNames.map(name => ({ name, spent: expenses.filter(e => e.category === name).reduce((s, e) => s + e.amount, 0), planned: 0 })),
    ].sort((a, b) => b.spent - a.spent);
    Charts.budgetCategoryBar(
      chartSorted.map(s => s.name),
      chartSorted.map(s => s.spent),
      chartSorted.map(s => s.planned),
      chartSorted.map(s => Utils.getCategoryColor(s.name))
    );
